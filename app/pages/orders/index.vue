...
<template>
  <div>
    <div class="max-w-[98%] mx-auto flex flex-col gap-5">
        ...
        <div class="flex items-center gap-4 min-w-0">
          ...
          <div class="min-w-0">
            ...
            <p class="text-[11px] text-outline font-mono mt-0.5">
              {{ meta ? `${meta.total} order${meta.total === 1 ? '' : 's'} · ${rangeLabel}` : rangeLabel }}
            </p>
          </div>
        </div>
      ...
      <!-- ADD ↓ — scope notice: today by default, one tap to widen -->
      <div v-if="isTodayScope"
        class="g-card flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 sm:px-6 py-3.5">
        <p class="text-xs sm:text-sm text-on-surface-variant flex items-start gap-2">
          <font-awesome-icon :icon="['fas', 'circle-info']" class="mt-0.5 text-ribbon-blue shrink-0" />
          <span>Showing <span class="font-bold text-on-surface">today's</span> orders only. Widen the range to see
            earlier work.</span>
        </p>
        <div class="flex flex-wrap items-center gap-2 shrink-0">
          <button type="button" class="preset-chip" @click="applyPreset('7d'); apply()">
            <font-awesome-icon :icon="['fas', 'calendar-week']" class="text-[10px]" />
      ...
      <div v-show="filtersOpen" class="g-card p-5 sm:p-6">
        <div class="flex items-center gap-3 mb-4">
          ...
          <span v-if="activeFilterCount"
            class="ml-auto text-[11px] font-bold text-ribbon-blue bg-ribbon-blue/10 px-2.5 py-1 rounded-full">
        ...
        <div class="g-card p-6 sm:p-8">
          ...
          <div v-if="meta && meta.total_pages > 1" class="flex items-center justify-between gap-3 mt-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <button type="button" class="pager-btn" :disabled="meta.page >= meta.total_pages"
      ...
      <div v-else class="g-card flex flex-col items-center justify-center gap-3 py-16 text-center">
        ...
        <p class="text-sm text-on-surface-variant">
          {{ isTodayScope ? 'No laboratory orders for today yet.' : 'No laboratory orders match your filters.' }}
        </p>
        <button v-if="isTodayScope" type="button" class="btn-reset" @click="applyPreset('all'); apply()">
...
// ADD ↓ — moved up from ~line 309 so it's initialised before `filters` (avoids TDZ)
...
const filters = reactive<Record<string, any>>({
  from: todayIso(), to: todayIso(),   // CHANGED — was from: '', to: ''
  q: '',
...
// ADD ↓ — true while the list is pinned to today's date only
const isTodayScope = computed(() => {
  const t = todayIso()
...
const activeFilterCount = computed(() => {
  let c = 0
  if (filters.q) c++
  // CHANGED — the today default is the baseline, not a filter the user set
  if (!isTodayScope.value) {
    if (filters.from) c++
    if (filters.to) c++
  }
...
const clearAll = () => {
  ...
  filters.sub_department_id = undefined
  filters.page = 1
  applyPreset('today')      // CHANGED — was from/to = '' + activePreset = 'all'
  load()
}
...
/* filter chrome */
.filter-toggle {
  @apply relative inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-semibold text-ribbon-blue bg-ribbon-blue/10 hover:bg-ribbon-blue/20 transition-colors;
}

.filter-badge {
  @apply inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 rounded-full text-[10px] font-bold text-white bg-primary-gradient;
}
