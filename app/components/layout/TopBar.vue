...
<template>
  <header
    class="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-white/80 backdrop-blur-md shadow-sm gap-3 shrink-0 min-w-0">

    <!-- ── Left: hamburger + title + search ──────────────────────────── -->
    <div class="flex items-center gap-3 sm:gap-5 min-w-0 flex-1">

      <!-- Hamburger (mobile) -->
      <button class="lg:hidden text-on-surface-variant hover:text-primary p-1 -ml-1 shrink-0 transition-colors"
      ...
      <!-- Page title -->
      <h1
        class="text-lg sm:text-xl md:text-2xl font-semibold sm:font-bold text-on-surface truncate hidden sm:block shrink-0">
        {{ pageTitle }}
      </h1>
      ...
      <div class="hidden md:block relative w-full max-w-xs min-w-0">
        ...
        <input type="text" placeholder="Search…" class="w-full pl-9 pr-3 py-2 text-sm rounded-full bg-surface-container
    ...
    <div class="flex items-center gap-2 sm:gap-3 shrink-0">
      ...
      <div class="relative" ref="userMenuContainer">
        ...
        -->
        <button class="flex items-center gap-2 pl-1 pr-2 sm:pr-3 py-1 rounded-full
                 hover:bg-surface-container transition-colors duration-200
                 focus:outline-none focus:ring-2 focus:ring-primary/30" :aria-expanded="userMenuOpen"
          aria-haspopup="true" @click="userMenuOpen = !userMenuOpen">
          <!-- Avatar: gradient circle with user initials -->
          <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center
        ...
        <Transition name="dropdown">
          <div v-if="userMenuOpen" class="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl
            ...
            <div class="px-4 py-3 flex items-center gap-3"
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-on-surface-variant truncate mt-0.5">{{ auth.user?.email ?? auth.user?.username }}
                </p>
                <!-- Role + department badges -->
                <div class="flex items-center gap-1.5 mt-1 flex-wrap">
                  <span v-if="auth.user?.role_label"
                    class="inline-block text-xs font-semibold text-[#614500] bg-[#DB9D00]/50 px-2 py-0.5 rounded-full truncate max-w-full">
                    {{ auth.user?.role_label }}
                  </span>
                  <span v-if="auth.user?.department"
                    class="inline-flex items-center gap-1 text-xs font-semibold text-[#1f3a5f] bg-[#3d7fbf]/15 px-2 py-0.5 rounded-full truncate max-w-full">
                    <font-awesome-icon :icon="['fas', 'building']" class="text-[10px]" />
            ...
            <div class="py-1.5">
              ...
              <!-- Settings -->
              <NuxtLink to="/admin/settings" class="flex items-center gap-3 px-4 py-2.5
                       text-sm font-medium text-on-surface
                       hover:bg-surface-low transition-colors duration-150" @click="userMenuOpen = false">
              ...
              <button class="w-full flex items-center gap-3 px-4 py-2.5
                       ...
                       hover:bg-error-container/40 transition-colors duration-150" @click="logout">
